...
import { FONTS, ACCENTS, THEMES } from '../hooks/useSettings.js'
      ...
      <div className="widget w-full max-w-sm p-6 scale-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-5">
          ...
          <div className="flex flex-wrap gap-2">
            {ACCENTS.map(a => (
              <button key={a.color} onClick={() => save({ accent: a.color })} title={a.label}
                style={{
                  width: 30, height: 30, borderRadius: '50%', background: a.color, cursor: 'pointer',
            ...
            <div className="relative">
              <input type="color" value={customColor}
                ...
                style={{ width:30, height:30, borderRadius:'50%', padding:0, border:'none', cursor:'pointer', opacity:0, position:'absolute', inset:0 }}
              />
              <div style={{ width:30, height:30, borderRadius:'50%', background:'conic-gradient(red,yellow,lime,cyan,blue,magenta,red)', outline:'1.5px solid rgba(0,0,0,0.1)', pointerEvents:'none' }} />
            </div>
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {FONTS.map(f => (
              <button key={f.id} onClick={() => save({ font: f.id })}
                ...
                <span style={{ fontFamily: f.id === 'pixel' ? "'Press Start 2P'" : f.id === 'fraunces' ? 'Fraunces' : f.id === 'dm' ? 'DM Sans' : f.id === 'mono-jb' ? 'JetBrains Mono' : 'Inter' }}>
                  {f.label}
                </span>
